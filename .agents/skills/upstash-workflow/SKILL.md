...
const log = debug('lobe-cloud:workflows:{workflow-name}');
...
export const { POST } = serve<ProcessPayload>(
  async (context) => {
    ...
    await context.run('{workflow}:trigger-paginate', () =>
      WorkflowClass.triggerPaginateItems({}),
    );
...
await Promise.all(
  items.map((item) =>
    context.run(`{workflow}:execute:${item.id}`, () =>
      processItem(item),
    ),
  ),
);
...
await Promise.all(
  items.map((item) =>
    context.run(`{workflow}:execute`, () => // ❌ Not unique
      processItem(item),
    ),
  ),
);
...
export const { POST } = serve<GenerateUserPlaceholderPayload>(
  async (context) => {
...
export const { POST } = serve<GenerateAgentWelcomePayload>(
  async (context) => {
...
await Promise.all(
  items.map(item => context.run('process', () => process(item)))
);
```
...
// Good: Unique step names
await Promise.all(
  items.map(item => context.run(`process:${item.id}`, () => process(item)))
);
```
...
await Promise.all(
  allItems.map(item => triggerExecute(item))
);
```
...
await Promise.all(
  itemsNeedingProcessing.map(item => triggerExecute(item))
);
```
